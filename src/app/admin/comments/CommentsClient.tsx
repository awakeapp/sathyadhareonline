      ...
      <PresenceCard className="bg-zinc-50 dark:bg-white/5 border-none">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-14 px-4 rounded-2xl bg-white dark:bg-zinc-950 border-none shadow-sm font-bold text-xs uppercase tracking-widest text-indigo-400 focus:ring-2 focus:ring-indigo-500/20">
            ...
            <select value={articleFilter} onChange={(e) => setArticleFilter(e.target.value)} className="h-14 px-4 rounded-2xl bg-white dark:bg-zinc-950 border-none shadow-sm font-bold text-xs uppercase tracking-widest text-indigo-400 focus:ring-2 focus:ring-indigo-500/20 max-w-[150px]">
      ...
      <div className="space-y-4">
        {filteredComments.length === 0 ? (
          <PresenceCard className="py-24 text-center border-dashed border-2 border-indigo-100 flex flex-col items-center">
            ...
            <p className="font-black text-xl text-zinc-500 uppercase tracking-widest">No Communications</p>
          </PresenceCard>
        ) : (
                  ...
                  <div className="p-5 flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                       <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
                          ...
                          <div>
                             <p className="font-black text-zinc-900 dark:text-zinc-50 truncate">{name}</p>
                             <div className="flex items-center gap-2 mt-0.5">
                                <Clock className="w-3 h-3 text-zinc-400" strokeWidth={1.25} />
                          ...
                          <div className="md:ml-auto flex items-center gap-2">
                             ...
                             {c.is_spam && (
                               <span className="px-3 py-1 rounded-lg text-[9px] bg-indigo-900/10 text-indigo-400 border border-indigo-500/20 font-black uppercase tracking-widest flex items-center gap-1">
                                  <ShieldAlert className="w-3 h-3" strokeWidth={1.25} /> SPAM
                               </span>
                             )}
                          </div>
                       </div>

                       <div className="bg-zinc-50 dark:bg-white/5 rounded-2xl p-4 mb-3 border-l-4 border-indigo-100 dark:border-indigo-500/20">
                    ...
                    <div className="shrink-0 flex md:flex-col gap-2 pt-1">
                       ...
                       {isPendingStatus && (
                         <button onClick={() => wrapAction(updateCommentStatusAction(c.id, 'rejected'), 'Target Quarantined')} className="w-12 h-12 rounded-xl bg-orange-400 text-white flex items-center justify-center hover:scale-105 active:scale-95 shadow-lg shadow-orange-500/20 transition-all">
                            <RejectIcon className="w-6 h-6" strokeWidth={1.25} />
                         </button>
                       )}
