        ...
        <form onSubmit={handleSearchSubmit} className="flex flex-col xl:flex-row gap-4 items-end">
          ...
          <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
             <div className="space-y-2">
               ...
               <div className="relative">
                 ...
                 <input 
                   ...
                   className="w-full h-12 pl-12 pr-4 rounded-2xl bg-white dark:bg-[#1b1929] border-none shadow-sm focus:ring-2 focus:ring-indigo-500/20 font-bold text-xs"
                 />
               </div>
             </div>
             ...
             <div className="space-y-2">
               ...
               <div className="relative">
                 <User className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300" />
                 <select 
                   value={userFilter} 
                   onChange={(e) => { setUserFilter(e.target.value); setPage(1); }} 
                   className="w-full h-12 pl-12 pr-4 rounded-2xl bg-white dark:bg-[#1b1929] border-none shadow-sm focus:ring-2 focus:ring-indigo-500/20 font-bold text-xs appearance-none"
                 >
             ...
             <div className="space-y-2">
               ...
               <div className="relative">
                 <Calendar className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300" />
                 <input 
                   type="date"
                   value={startDate}
                   onChange={(e) => { setStartDate(e.target.value); setPage(1); }}
                   className="w-full h-12 pl-12 pr-4 rounded-2xl bg-white dark:bg-[#1b1929] border-none shadow-sm focus:ring-2 focus:ring-indigo-500/20 font-bold text-xs text-indigo-400"
                 />
               </div>
             </div>
               ...
               {isPending && logs.length === 0 ? (
                 <tr>
                    <td colSpan={4} className="py-24 text-center">
                       ...
                       <p className="text-gray-300 font-black uppercase tracking-widest text-[10px]">Decrypting History...</p>
                    </td>
                 </tr>
               ) : logs.length === 0 ? (
                 <tr>
                    <td colSpan={4} className="py-24 text-center">
                       ...
                       <p className="font-black text-[#1b1929] dark:text-white uppercase tracking-widest">Protocol Void</p>
                       <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-2">No activity records found</p>
                    </td>
                 </tr>
               ) : (
                 logs.map((log) => {
                       ...
                       <tr className={`group transition-all hover:bg-gray-50/50 dark:hover:bg-white/5 ${isExpanded ? 'bg-indigo-50/30' : ''}`}>
                         ...
                         <td className="px-4 py-5">
                           <div className="flex items-center gap-3">
                             ...
                             <div>
                               <p className="font-black text-[#1b1929] dark:text-white text-sm">{actorName}</p>
                         ...
                         <td className="px-4 py-5 text-right">
                           {hasDetails ? (
                             <button 
                               ...
                               className={`h-9 px-5 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${isExpanded ? 'bg-[#5c4ae4] text-white shadow-lg' : 'bg-white dark:bg-[#1b1929] text-gray-400 hover:text-[#5c4ae4] shadow-sm'}`}
                             >
                                <Eye className="w-3 h-3 inline mr-2" /> {isExpanded ? 'Retract' : 'Analyze'}
                             </button>
                           ) : (
                             <span className="text-[9px] font-black text-gray-200 uppercase tracking-widest">Null Payload</span>
                           )}
                         </td>
                       </tr>
        ...
        <div className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/30 dark:bg-white/5 border-t border-indigo-50 dark:border-white/5">
           <div className="flex items-center gap-4">
             ...
             <select 
               ...
               className="h-10 px-4 rounded-xl bg-white dark:bg-[#1b1929] border-none shadow-sm text-[10px] font-black text-[#5c4ae4]"
             >
           ...
           <div className="flex items-center gap-4">
             ...
             <div className="flex gap-3">
                <button 
                  ...
                  className="w-11 h-11 rounded-xl bg-white dark:bg-[#1b1929] text-gray-300 hover:text-[#5c4ae4] disabled:opacity-30 shadow-sm border-none flex items-center justify-center transition-all"
                >
